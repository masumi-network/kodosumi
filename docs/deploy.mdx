...
<Warning>
This page documents the legacy deployment system using `koco deploy` and YAML config files. This approach has been replaced by the **Expose/Boot system** which is managed through the Admin Panel UI. See [Expose Management](/expose) and [Boot API](/boot-api) for the current deployment workflow.
</Warning>
...
<Note>The deployment API at `/deploy` and `/serve` is experimental.</Note>
