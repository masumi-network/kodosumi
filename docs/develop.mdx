...
<Note>Use the _flat package layout_. The _flat layout_ simplifies the deployment process as we will see later in [deployment](/deploy). In contrast to the _src layout_ the the _flat layout_ does **not** need any additional installation step to be _importable_ by Python.</Note>
