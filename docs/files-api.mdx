...
response = await client.delete(f"/files/cancel/{upload_id}")
```
...
```python
# DELETE /files/{fid}/{path}
response = await client.delete(f"/files/{fid}/out/report.pdf")
```

Deletes a file or directory within an execution's file space.
