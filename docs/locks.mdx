...
Kodosumi provides a powerful Human-in-the-Loop (HITL) mechanism through its lock system, allowing automated workflows to pause and request human intervention when needed. This document explains how to implement HITL services using locks and leases.

<Note>See also the [`kodosumi-examples` repository](https://github.com/masumi-network/kodosumi-examples) on HITL.</Note>
...
Here's a complete HITL service implementation:
