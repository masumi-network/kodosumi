...
The following input types are also available and follow the same pattern as `InputText`:

| Type | HTML Input | Description |
|------|-----------|-------------|
| `InputEmail` | `email` | Email address with browser validation |
| `InputUrl` | `url` | URL with browser validation |
| `InputTel` | `tel` | Phone number |
| `InputSearch` | `search` | Search input with clear button |
| `InputMonth` | `month` | Month/year picker |
| `InputWeek` | `week` | Week picker |
| `InputColor` | `color` | Color picker |
| `InputRange` | `range` | Slider with min/max/step |
| `InputHidden` | `hidden` | Hidden field (not visible to user) |
| `InputRadio` | `radio` | Radio button group |
